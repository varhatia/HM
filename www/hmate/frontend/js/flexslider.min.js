(function(e) {
	e.flexslider = function(t, n) {
		var r = e(t), i = e.extend({}, e.flexslider.defaults, n), s = i.namespace, o = "ontouchstart" in window
				|| window.DocumentTouch && document instanceof DocumentTouch, u = o ? "touchend"
				: "click", a = i.direction === "vertical", f = i.reverse, l = i.itemWidth > 0, c = i.animation === "fade", h = i.asNavFor !== "", p = {};
		...
		p = {
			init : function() {
				...
				r.containerSelector = i.selector.substr(0, i.selector
						.search(" "));
				...
				if (i.controlsContainer !== "")
					r.controlsContainer = e(i.controlsContainer).length > 0
							&& e(i.controlsContainer);
				if (i.manualControls !== "")
					r.manualControls = e(i.manualControls).length > 0
							&& e(i.manualControls);
				if (i.randomize) {
				...
				if (i.keyboard
						&& (e(r.containerSelector).length === 1 || i.multipleKeyboard)) {
					e(document).bind(
							"keyup",
							function(e) {
								var t = e.keyCode;
								if (!r.animating && (t === 39 || t === 37)) {
									var n = t === 39 ? r.getTarget("next")
											: t === 37 ? r.getTarget("prev")
													: false;
				...
				if (i.slideshow) {
					...
					i.initDelay > 0 ? setTimeout(r.play, i.initDelay) : r
							.play()
				}
				if (o && i.touch)
					p.touch();
			...
			asNav : {
				setup : function() {
					...
					r.slides.removeClass(s + "active-slide").eq(r.currentItem)
							.addClass(s + "active-slide");
					r.slides.click(function(t) {
						t.preventDefault();
						var n = e(this), s = n.index();
						if (!e(i.asNavFor).data("flexslider").animating
								&& !n.hasClass("active")) {
							r.direction = r.currentItem < s ? "next" : "prev";
							r
									.flexAnimate(s, i.pauseOnAction, false,
											true, true)
						}
					})
				}
			...
			controlNav : {
				...
				setupPaging : function() {
					var t = i.controlNav === "thumbnails" ? "control-thumbs"
							: "control-paging", n = 1, a;
					r.controlNavScaffold = e('<ol class="' + s + "control-nav "
							+ s + t + '"></ol>');
					if (r.pagingCount > 1) {
						for ( var f = 0; f < r.pagingCount; f++) {
							a = i.controlNav === "thumbnails" ? '<img src="'
									+ r.slides.eq(f).attr("data-thumb") + '"/>'
									: "<a>" + n + "</a>";
					...
					if (o) {
						r.controlNavScaffold.delegate("a", "click touchstart",
								function(e) {
									e.preventDefault()
								})
					}
				...
				setupManual : function() {
					...
					r.controlNav.live(u, function(t) {
						...
						var n = e(this), o = r.controlNav.index(n);
						if (!n.hasClass(s + "active")) {
							o > r.currentSlide ? r.direction = "next"
									: r.direction = "prev";
							r.flexAnimate(o, i.pauseOnAction)
						}
					});
				...
				set : function() {
					...
					r.controlNav = e("." + s + "control-nav li " + t,
							r.controlsContainer ? r.controlsContainer : r)
				},
				active : function() {
					r.controlNav.removeClass(s + "active").eq(r.animatingTo)
							.addClass(s + "active")
				},
				update : function(t, n) {
					if (r.pagingCount > 1 && t === "add") {
						r.controlNavScaffold.append(e("<li><a>" + r.count
								+ "</a></li>"))
			...
			directionNav : {
				setup : function() {
					var t = e('<ul class="' + s
							+ 'direction-nav"><li><a class="' + s
							+ 'prev" href="#">' + i.prevText
							+ '</a></li><li><a class="' + s + 'next" href="#">'
							+ i.nextText + "</a></li></ul>");
					if (r.controlsContainer) {
						e(r.controlsContainer).append(t);
						r.directionNav = e("." + s + "direction-nav li a",
								r.controlsContainer)
					} else {
						r.append(t);
					...
					r.directionNav.bind(u, function(t) {
						t.preventDefault();
						var n = e(this).hasClass(s + "next") ? r
				...
				update : function() {
					...
					if (!i.animationLoop) {
						...
						} else if (r.animatingTo === 0) {
							r.directionNav.removeClass(e).filter(
									"." + s + "prev").addClass(e)
			...
			pausePlay : {
				...
				update : function(e) {
					e === "play" ? r.pausePlay.removeClass(s + "pause")
							.addClass(s + "play").text(i.playText)
							: r.pausePlay.removeClass(s + "play").addClass(
									s + "pause").text(i.pauseText)
				}
			},
			touch : function() {
				function d(u) {
					if (r.animating) {
						...
						o = a ? r.h : r.w;
						h = Number(new Date);
						s = l && f && r.animatingTo === r.last ? 0
								: l && f ? r.limit - (r.itemW + i.itemMargin)
										* r.move * r.animatingTo
										: l && r.currentSlide === r.last ? r.limit
												: l ? (r.itemW + i.itemMargin)
														* r.move
														* r.currentSlide
														: f ? (r.last
				...
				function v(t) {
					...
					p = a ? Math.abs(u) < Math.abs(t.touches[0].pageX - n)
							: Math.abs(u) < Math.abs(t.touches[0].pageY - n);
				...
				function m(a) {
					if (r.animatingTo === r.currentSlide && !p && !(u === null)) {
						var l = f ? -u : u, c = l > 0 ? r.getTarget("next") : r
								.getTarget("prev");
						if (r.canAdvance(c)
								&& (Number(new Date) - h < 550
										&& Math.abs(l) > 20 || Math.abs(l) > o / 2)) {
							r.flexAnimate(c, i.pauseOnAction)
						} else {
							r
									.flexAnimate(r.currentSlide,
											i.pauseOnAction, true)
						}
					}
		...
		r.flexAnimate = function(t, n, o, u, d) {
			if (!r.animating && (r.canAdvance(t) || o) && r.is(":visible")) {
				if (h && u) {
					var v = e(i.asNavFor).data("flexslider");
					r.atEnd = t === 0 || t === r.count - 1;
					v.flexAnimate(t, true, false, true, d);
					r.direction = r.currentItem < t ? "next" : "prev";
					v.direction = r.direction;
					if (Math.ceil((t + 1) / r.visible) - 1 !== r.currentSlide
							&& t !== 0) {
						r.currentItem = t;
						r.slides.removeClass(s + "active-slide").eq(t)
								.addClass(s + "active-slide");
				...
				if (i.controlNav)
					p.controlNav.active();
				if (!l)
					r.slides.removeClass(s + "active-slide").eq(t).addClass(
							s + "active-slide");
				...
				if (!c) {
					var m = a ? r.slides.filter(":first").height()
							: r.computedW, g, y, b;
					if (l) {
						g = i.itemWidth > r.w ? i.itemMargin * 2 : i.itemMargin;
						b = (r.itemW + g) * r.move * r.animatingTo;
						y = b > r.limit && r.visible !== 1 ? r.limit : b
					} else if (r.currentSlide === 0 && t === r.count - 1
							&& i.animationLoop && r.direction !== "next") {
						y = f ? (r.count + r.cloneOffset) * m : 0
					} else if (r.currentSlide === r.last && t === 0
							&& i.animationLoop && r.direction !== "prev") {
						y = f ? 0 : (r.count + 1) * m
					} else {
						y = f ? (r.count - 1 - t + r.cloneOffset) * m
								: (t + r.cloneOffset) * m
					}
					r.setProps(y, "", i.animationSpeed);
					if (r.transitions) {
						...
						r.container.animate(r.args, i.animationSpeed, i.easing,
								function() {
									r.wrapup(m)
								})
					}
				} else {
					r.slides.eq(r.currentSlide).fadeOut(i.animationSpeed,
							i.easing);
		...
		r.wrapup = function(e) {
			if (!c && !l) {
				if (r.currentSlide === 0 && r.animatingTo === r.last
						&& i.animationLoop) {
					r.setProps(e, "jumpEnd")
				} else if (r.currentSlide === r.last && r.animatingTo === 0
						&& i.animationLoop) {
		...
		r.canAdvance = function(e) {
			var t = h ? r.pagingCount - 1 : r.last;
			return h && r.currentItem === 0 && e === r.pagingCount - 1
					&& r.direction !== "next" ? false : e === r.currentSlide
					&& !h ? false : i.animationLoop ? true : r.atEnd
					&& r.currentSlide === 0 && e === t
					&& r.direction !== "next" ? false : r.atEnd
					&& r.currentSlide === t && e === 0
					&& r.direction === "next" ? false : true
		};
		...
		r.setProps = function(e, t, n) {
			var s = function() {
				var n = e ? e : (r.itemW + i.itemMargin) * r.move
						* r.animatingTo, s = function() {
					if (l) {
						return t === "setTouch" ? e : f
								&& r.animatingTo === r.last ? 0 : f ? r.limit
								- (r.itemW + i.itemMargin) * r.move
								* r.animatingTo
								: r.animatingTo === r.last ? r.limit : n
					} else {
						switch (t) {
						case "setTotal":
							return f ? (r.count - 1 - r.currentSlide + r.cloneOffset)
									* e
			...
			if (r.transitions) {
				s = a ? "translate3d(0," + s + ",0)" : "translate3d(" + s
						+ ",0,0)";
		...
		r.setup = function(t) {
			if (!c) {
				var n, o;
				if (t === "init") {
					r.viewport = e('<div class="flex-viewport"></div>').css({
						overflow : "hidden",
						position : "relative"
				...
				if (i.animationLoop && !l) {
					...
					r.container.append(
							r.slides.first().clone().addClass("clone"))
							.prepend(r.slides.last().clone().addClass("clone"))
				}
				r.newSlides = e(i.selector, r);
				n = f ? r.count - 1 - r.currentSlide + r.cloneOffset
						: r.currentSlide + r.cloneOffset;
				if (a && !l) {
				...
				if (t === "init")
					r.slides.eq(r.currentSlide).fadeIn(i.animationSpeed,
							i.easing);
				if (i.smoothHeight)
					p.smoothHeight()
			}
			if (!l)
				r.slides.removeClass(s + "active-slide").eq(r.currentSlide)
						.addClass(s + "active-slide")
		};
		r.doMath = function() {
			...
			if (l) {
				...
				r.itemW = r.minW > r.w ? (r.w - t * n) / n
						: r.maxW < r.w ? (r.w - t * s) / s
								: i.itemWidth > r.w ? r.w : i.itemWidth;
				...
				r.limit = r.pagingCount === 1 ? 0
						: i.itemWidth > r.w ? (r.itemW + t * 2) * r.count - r.w
								- t : (r.itemW + t) * r.count - r.w
			} else {
				r.itemW = r.w;
		...
		r.addSlide = function(t, n) {
			...
			r.last = r.count - 1;
			if (a && f) {
				n !== undefined ? r.slides.eq(r.count - n).after(s)
						: r.container.prepend(s)
			} else {
				n !== undefined ? r.slides.eq(n).before(s) : r.container
						.append(s)
			}
			r.update(n, "add");
